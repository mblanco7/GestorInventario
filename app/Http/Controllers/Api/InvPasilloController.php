...
class InvPasilloController extends Controller
{

    use AuthorizesRequests, DispatchesJobs, ValidatesRequests;

    private InvPasillosService $service;

    public function __construct(InvPasillosService $service)
    {
    ...
    function guardar(Request $request) {
        ...
        $objeto = new InvPasillo($requestData);
        try {
            $objeto->id = null;
            $response->objeto = $this->service->save($objeto);
            $response->data = true;
            $response->mensaje = "Pasillo almacenada satisfactoriamente";
            return $this->return200($response);
        } catch (Throwable $th) {
            $response->data = false;
            $response->mensaje = "Problemas al intentar almacenar la pasillo. ".$th->getMessage();
    ...
    function actualizar(Request $request) {
        ...
        try {
            ...
            $response->objeto = $this->service->save($objeto);
            $response->data = true;
            $response->mensaje = "Pasillo actualizada satisfactoriamente";
            return $this->return200($response);
        } catch (Throwable $th) {
            $response->data = false;
            $response->mensaje = "Problemas al intentar actualizar la pasillo. ".$th->getMessage();
            return $this->return500($response);
        }        
    }
    ...
    function borrar(Request $request) {
        ...
        try {
            ...
            if ($this->service->delete($objeto)) {
                $response->data = true;
                $response->mensaje = "Pasillo eliminada satisfactoriamente";
                return $this->return200($response);
            } else {
                throw new Exception();
            }
            ...
            $response->data = false;
            $response->mensaje = "Problemas al intentar borrar la pasillo. ".$th->getMessage();
            return $this->return500($response);
        }
    }
