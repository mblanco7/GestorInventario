...
class ArqPerfilController extends Controller
{
    ...
    private ArqPerfilesService $service;

    public function __construct(ArqPerfilesService $service)
    {
    ...
    function guardar(Request $request) {
        ...
        $objeto = new ArqPerfil($requestData);
        try {
            $objeto->id = null;
            $response->objeto = $this->service->save($objeto);
            $response->data = true;
            $response->mensaje = "Perfil almacenada satisfactoriamente";
            return $this->return200($response);
        } catch (Throwable $th) {
            $response->data = false;
            $response->mensaje = "Problemas al intentar almacenar la perfil. ".$th->getMessage();
    ...
    function actualizar(Request $request) {
        ...
        try {
            ...
            $response->objeto = $this->service->save($objeto);
            $response->data = true;
            $response->mensaje = "Perfil actualizada satisfactoriamente";
            return $this->return200($response);
        } catch (Throwable $th) {
            $response->data = false;
            $response->mensaje = "Problemas al intentar actualizar la perfil. ".$th->getMessage();
            return $this->return500($response);
        }        
    }
    ...
    function borrar(Request $request) {
        ...
        try {
            ...
            if ($this->service->delete($objeto)) {
                $response->data = true;
                $response->mensaje = "Perfil eliminada satisfactoriamente";
                return $this->return200($response);
            } else {
                throw new Exception();
            }
            ...
            $response->data = false;
            $response->mensaje = "Problemas al intentar borrar la perfil. ".$th->getMessage();
            return $this->return500($response);
        }
    }
