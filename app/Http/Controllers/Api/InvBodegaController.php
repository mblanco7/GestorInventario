...
class InvBodegaController extends Controller
{
    ...
    public function __construct(InvBodegasService $service)
    {
    ...
    function guardar(Request $request) {
        ...
        $objeto = new InvBodega($requestData);
        try {
            $objeto->id = null;
            $response->objeto = $this->service->save($objeto);
            $response->data = true;
            $response->mensaje = "Bodega almacenada satisfactoriamente";
            return $this->return200($response);
        } catch (Throwable $th) {
            $response->data = false;
            $response->mensaje = "Problemas al intentar almacenar la bodega. ".$th->getMessage();
    ...
    function actualizar(Request $request) {
        ...
        try {
            ...
            $response->objeto = $this->service->save($objeto);
            $response->data = true;
            $response->mensaje = "Bodega actualizada satisfactoriamente";
            return $this->return200($response);
        } catch (Throwable $th) {
            $response->data = false;
            $response->mensaje = "Problemas al intentar actualizar la bodega. ".$th->getMessage();
            return $this->return500($response);
        }        
    }
    ...
    function borrar(Request $request) {
        ...
        try {
            ...
            if ($this->service->delete($objeto)) {
                $response->data = true;
                $response->mensaje = "Bodega eliminada satisfactoriamente";
                return $this->return200($response);
            } else {
                throw new Exception();
            }
            ...
            $response->data = false;
            $response->mensaje = "Problemas al intentar borrar la bodega. ".$th->getMessage();
            return $this->return500($response);
        }
    }
